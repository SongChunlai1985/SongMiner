...
// -a sha256d -o stratum+tcp://ss.antpool.com:3333 -u chunlai.1
// bc1qjejx532sxjarp368t02vpm5e9440emlg6pzjnx
...
bool opt_debug = /*false*/ true;
bool opt_protocol = /*false*/ true;
...
static bool workio_get_work(struct workio_cmd *wc, CURL *curl)
{
	...
	while (!get_upstream_work(curl, ret_work)) {
        ...
        applog(LOG_ERR, &"json_rpc_call failed, retry after %d seconds [%d]",
            opt_fail_pause , wc->cmd);
...
static void *workio_thread(void *userdata)
{
	...
	while (ok) {
        ...
        case WC_GET_WORK:
            ...
            applog(LOG_ERR, "GetWork:" ,wc->u.work->txs);
            break;
        case WC_SUBMIT_WORK:
            ok = workio_submit_work(wc, curl);
            applog(LOG_ERR, "SubmitWork:" ,wc->u.work->txs);
            break;
...
#ifdef DEBUG
        struct stratum_ctx stratum_ = stratum;
        char *rpc_user_ = rpc_user;
        char *rpc_pass_ = rpc_pass;
        pthread_mutex_t g_work_lock_ = g_work_lock;
        int opt_retries_ = opt_retries;

        struct work g_work_ = g_work;
        time_t g_work_time_ = g_work_time;
#endif
